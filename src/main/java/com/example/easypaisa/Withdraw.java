...
public class Withdraw {
    ...
    public void show(Stage stage, UserStore.User user) {
        HBox header = H.header("Withdraw Cash",
        ...
        Button wdBtn = H.btn("Withdraw", Styles.BTN);
        wdBtn.setOnAction(e -> {
            double amt;
            ...
            if (amt > 50000)     { H.setMsg(msg, "Max withdrawal per transaction: Rs. 50,000.", true); return; }
            new PinDialog().show(stage, user, () -> {
                UserStore.addTx(user.phone, "-", "Cash Withdrawal", amt);
                H.setMsg(msg, "Rs. " + UserStore.fmt(amt) + " withdrawn! Balance: Rs."
